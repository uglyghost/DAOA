...
main(int argc, char* argv[])
{
      ...
      if (i==0)
        {
          ...
          std::cout << "node1: " << i << " rInd: " << j << std::endl;
        }
      else
      {
          rInd = rand()%(m_node - j) + j;
          //std::cout << "node1: " << i << " rInd: " << rInd << std::endl;
          p2p.Install(nodes.Get(i), nodes.Get(rInd));
      }
    }
  }
  ...
  int request_time = 5;
  double arrival_time[m_node][request_time];
  int arrival_seg[m_node][request_time];
  int rTim;
  int rSeg;
  ...
  {  
    for(int j = 0; j < request_time; j++)
    {
        if (j==0)
          ...
          {
            rTim = rand()%simulation_time;
            arrival_time[i][j] = rTim;
          }

        rSeg = rand()%8;
        arrival_seg[i][j] = rSeg;
    }
  }
