...
main(int argc, char* argv[])
{
  ...
  int m_cache_size = 0;
  int timeNum = 10;
  int delta = simulation_time/m_cnode + 1;
  ...
  {  
    for(int j = 0 ; j < request_time; j++)
    {
        ...
        int stage;
        stage = m_cnode/timeNum;
        arrival_time[i][j] = delta*stage;
        arrival_seg[i][j] = 1;
    }
  }
  ...
  */
  ...
  for (int i = 0 ; i < m_cnode; i++)
    {
      for (int j = 0 ; j < request_time; j++) 
      {
        ...
        consumerHelper.SetAttribute("CongestionControl", BooleanValue(false));
  ...
  std::string DASH_output = "TheroyDASH-N" + to_string(m_node) + "-S" + to_string(m_server) + "-R" + to_string(m_run) + "-S" + to_string(m_sel) +"-Mult-F.txt";
