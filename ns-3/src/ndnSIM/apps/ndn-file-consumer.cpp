...
namespace ns3 {
namespace ndn {
std::string fileName1 = "linkbitrate.txt"; //by cxy
...
FileConsumer::OnData(shared_ptr<const Data> data)
{
  ...
  if (ns3PacketTag != nullptr) {
    ...
    FwBijTag FwBIJTag;
    FwCapacityTag CapacityTag;
    FwFaceIdTag FaceIdTag;
    if (ns3PacketTag->getPacket()->PeekPacketTag(hopCountTag)) {
      hopCount = hopCountTag.Get();
      NS_LOG_DEBUG("Hop count: " << hopCount);
    }
    if (ns3PacketTag->getPacket()->PeekPacketTag(FwBIJTag)) {
      double m_nowtime = ns3::Simulator::Now().GetSeconds();
		  double m_passtime = FwBIJTag.Get();
      linkPrice = m_nowtime - m_passtime;
      //std::cout << "m_nowtime: " << m_nowtime << "\t m_passtime: " << m_passtime <<std::endl;
      NS_LOG_DEBUG("Link Price: " << linkPrice);      
    }
    if (ns3PacketTag->getPacket()->PeekPacketTag(CapacityTag)) {
      Capacity = CapacityTag.GetCapacity();
      NS_LOG_DEBUG("Capacity: " << Capacity);
    }
    if (ns3PacketTag->getPacket()->PeekPacketTag(FaceIdTag)) {
      FaceId = FaceIdTag.Get();
      NS_LOG_DEBUG("Face Id: " << FaceId);
    }

    std::map<uint32_t,double>::iterator iter1 = m_delayMap.find(FaceId);
    if(iter1 != m_delayMap.end())
      m_delayMap[FaceId] = (3/linkPrice)/250.0;
    else
      m_delayMap.insert(pair<uint32_t, double>(FaceId, (3/linkPrice)/250.0));
    
    std::map<uint32_t,uint32_t>::iterator iter2 = m_bitrateMap.find(FaceId);
    if(iter2 != m_bitrateMap.end())
      m_bitrateMap[FaceId] = Capacity;
    else
      m_bitrateMap.insert(pair<uint32_t, uint32_t>(FaceId, Capacity));
    
    fout << Simulator::Now().ToDouble(Time::S) << "\t" << GetNode ()->GetId() << "\t";
    for(const auto &it : m_delayMap)
    {
      if(it.first)
      {
        sumDelay = sumDelay + it.second;
        fout << it.second << "\t";
      }
    }
    fout << std::endl;
    for(const auto &it : m_bitrateMap)
    {
      if(it.first)
      {
        sumBitrate = sumBitrate + it.second;
      }
    }
    sumBitrate = 5;
    //std::cout << "sumBitrate: " << sumBitrate << "\t sumDelay: " << sumDelay << std::endl;
    if(sumDelay>sumBitrate)
      sumDelay = sumBitrate;
  }

  //std::cout << "link price:" << linkPrice << std::endl;
  ...
  if (!m_hasReceivedManifest)
  {
    ...
    if (lastPostfix == m_manifestPostfix)
    {
      ...
      } else
      {
        ...
        OnManifest(fileSize);
        AfterData(true, false, 0, sumDelay);
        // ScheduleNext(linkPrice);
      }
  ...
  AfterData(false, false, seqNo, sumDelay);
  // ScheduleNext(linkPrice);
}


void
FileConsumer::AfterData(bool manifest, bool timeout, uint32_t seq_nr, double price)
{
  if (AreAllSeqReceived())
  {
    OnFileReceived(0, 0);
  } else {
      ScheduleNextSendEvent(price);
  }
}

/*
void
FileConsumer::ScheduleNext(double linkPrice)
{
  if (AreAllSeqReceived())
  {
    //OnFileReceived(0, 0);
  } else {
      ScheduleNextSendEvent(linkPrice);
  }
}
*/
