...
protected:
  ...
  virtual void
  AfterData(bool manifest, bool timeout, uint32_t seq_nr, double price); // triggered AFTER OnFileData and AFTER Manifest

/*
  virtual void
  ScheduleNext(double linkPrice);
*/
  ...
  std::map<uint32_t,uint32_t> m_bitrateMap;
