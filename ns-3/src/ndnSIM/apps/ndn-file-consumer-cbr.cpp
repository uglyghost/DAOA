  ...
  static TypeId tid =
    TypeId("ns3::ndn::FileConsumerCbr")
      ...
      .AddAttribute("StartWindowSize", "The amount of interests that are allowed to be issued at the beginning of the download without knowing the actual file size (0 = none)", UintegerValue(0),
                    ...
                    MakeUintegerChecker<uint32_t>())
      .AddAttribute("CongestionControl", "Open the congestion cotrol", BooleanValue(true),
                    MakeBooleanAccessor (&FileConsumerCbr::m_congestionControl), 
                    MakeBooleanChecker ())
      .AddTraceSource("FileWindowSize", "Trace called every time a window size change",
                      MakeTraceSourceAccessor(&FileConsumerCbr::m_FileWindowSize));
    ;
...
FileConsumerCbr::StartApplication()
{
  ...
  if (m_windowSize == 0) // means: determine!
  {
    ...
    //std::cout << "Bitrate: " << bitrate << ", max_packets: " << max_packets_possible << std::endl;
...
void
FileConsumerCbr::AfterData(bool manifest, bool timeout, uint32_t seq_nr, double bitrate)
{
  ...
  if(bitrate>=1&&m_congestionControl)
  {
    SetWindowSize( floor( ((pow(10,6)*bitrate)/8.0)/(double)GetFaceMTU(0)) );
    //std::cout << "bitrate: "<<bitrate<< "\t Mtu: " << GetFaceMTU(0) << "\t windowsize: " << floor( ((pow(10,6)*bitrate)/8.0)/(double)GetFaceMTU(0)) << std::endl;
  }
  if (manifest)
  {
    SendPacket();
...
FileConsumerCbr::SendPacket()
{
  ...
  if(m_timestamp < Simulator::Now().ToDouble(Time::S))
  {
    m_FileWindowSize(this, m_windowSize);
    m_timestamp =  floor(Simulator::Now().ToDouble(Time::S)) + 0.1;
    //std::cout << "timestamp: " << m_timestamp << " simulation now: " << Simulator::Now().ToDouble(Time::S) << " windowsize: "<< m_windowSize << std::endl;
  }
  //std::cout << "timestamp: " << m_timestamp << " simulation now: " << Simulator::Now().ToDouble(Time::S) << std::endl;
