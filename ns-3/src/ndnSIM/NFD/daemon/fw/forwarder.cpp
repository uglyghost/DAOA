  ...
  if (!isPending) {
    if (m_csFromNdnSim == nullptr) {
      ...
      if (match != nullptr) {
		...
		//std::cout<<inFace.m_facebij<<std::endl;
      }
    }
  }
  else {
                             ...
                             const Data& data)
{
  ...
  Face& outFace = *const_pointer_cast<Face>(inFace.shared_from_this());
  //outFace.m_facebij = inFace.m_facebij;
  //std::cout << "outFace.m_facebij: " << outFace.m_facebij << std::endl;
  //std::cout<<"outFace" << &outFace << "inface"<< &inFace<<std::endl;	
...
Forwarder::onIncomingData(Face& inFace, const Data& data)
{
  ...
  for (std::set<shared_ptr<Face> >::iterator it = pendingDownstreams.begin();
	...
	//pendingDownstream->m_facebij=10;
	//std::cout<<"pendingDownstream: "<<pendingDownstream<<std::endl;
	//inFace.m_facebij = pendingDownstream->m_facebij;
	//std::cout<<"forwarder in Face pending: "<< pendingDownstream <<std::endl;
	//std::cout<<"forwarder in Face inFace: "<< &inFace <<std::endl;
	
    // goto outgoing Data pipeline
	//std::cout<<"outFace.m_facebij"<<(*pendingDownstream).m_facebij<<std::endl;
	//std::cout<<"outFace.m_prefacebij"<<(*pendingDownstream).m_prefacebij<<std::endl;
