...
public:
  ...
  Face(const FaceUri& remoteUri, const FaceUri& localUri,
       bool isLocal = false, bool isMultiAccess = false, double FaceBij = 10000, double PreFaceBij = 0, int Queue = 0, int PreQueue = 0, int PPreQueue = 0);  // add FaceBij
...
public: // attributes
  ...
  bool serverIndex;
  double m_facebij;
  std::map<uint32_t,double> m_delayMap;
  uint32_t m_capacity;
  double m_prefacebij;
  int m_queue;
  int m_prequeue;
...
/*
  void
  SetPara(double FaceBij,double PreFaceBij, int PreQueue, int PPreQueue)
  {
    this->m_facebij = FaceBij;
	this->m_prefacebij = PreFaceBij;
	this->m_prequeue = PreQueue;
	this->m_pprequeue = PPreQueue;
  }
*/
