...
Entry::insertOrUpdateInRecord(shared_ptr<Face> face, const Interest& interest)
{
  ...
  if (it == m_inRecords.end()) {
	...
	//face->m_facebij =11;
	//std::cout<<face->m_facebij<<std::endl;
  }
  //std::cout<<"insertOrUpdateInRecord In: "<< face <<std::endl;
  it->update(interest);
  return it;
}
...
{
  //std::cout<<"getInRecord: "<< &face <<std::endl;
...
{
  //std::cout<<"insertOrUpdateInRecord Out: "<< &face <<std::endl;
...
{
  //std::cout<<"getOutRecord: "<< &face <<std::endl;
...
{
  //std::cout<<"deleteOutRecord: "<< &face <<std::endl;
  auto it = std::find_if(m_outRecords.begin(), m_outRecords.end(),
    [&face] (const OutRecord& outRecord) { return outRecord.getFace().get() == &face; });
  if (it != m_outRecords.end()) {
