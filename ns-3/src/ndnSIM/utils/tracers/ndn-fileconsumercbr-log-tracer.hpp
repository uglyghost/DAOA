...
#ifndef NDN_FILECONSUMERCBR_LOG_TRACER_H
#define NDN_FILECONSUMERCBR_LOG_TRACER_H
 ...
 */
class FileConsumerCbrLogTracer : public SimpleRefCount<FileConsumerCbrLogTracer> {
public:
  /**
   ...
   *          for the lifetime of simulation, otherwise SEGFAULTs are inevitable
   */
  static Ptr<FileConsumerCbrLogTracer>
   ...
   */
  FileConsumerCbrLogTracer(shared_ptr<std::ostream> os, Ptr<Node> node);
   ...
   */
  FileConsumerCbrLogTracer(shared_ptr<std::ostream> os, const std::string& node);

  /**
   * @brief Destructor
   */
  ~FileConsumerCbrLogTracer();
...
private:
  ...
  void
  FileWindowSizeTrace(Ptr<ns3::ndn::App> app, double windowSize);
